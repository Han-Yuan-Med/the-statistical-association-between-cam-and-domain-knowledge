...
for id in range(len(dataset_id)):
    ...
    results_df.append([f"VGG-11", f"{id}", f"Grad-CAM", f"Optic cup",
                       f"{xai_oc_mean} ({xai_oc_std})", f"{oc_fundus_mean} ({oc_fundus_mean})",
    ...
    results_df.append([f"VGG-11", f"{id}", f"Grad-CAM", f"Optic disk",
                       ...
                       f"{od_fundus_mean} ({od_fundus_mean})",
    ...
    results_df.append([f"VGG-11", f"{id}", f"Grad-CAM", f"Blood vessels",
                       ...
                       f"{bv_fundus_mean} ({bv_fundus_mean})",
...
xai_oc_mean, xai_oc_std = bootstrap_sample(value_list=np.array(xai_oc_list_full), times=100)
oc_fundus_mean, oc_fundus_std = bootstrap_sample(value_list=np.array(oc_fundus_list_full), times=100)
oc_difference_mean, oc_difference_std = bootstrap_sample(value_list=np.array(xai_oc_list_full) -
                                                                    np.array(oc_fundus_list_full), times=100)
iou_oc_mean, iou_oc_std = bootstrap_sample(value_list=np.array(iou_oc_list_full), times=100)
dice_oc_mean, dice_oc_std = bootstrap_sample(value_list=np.array(dice_oc_list_full), times=100)
hau_oc_mean, hau_oc_std = bootstrap_sample(value_list=np.array(hau_oc_list_full), times=100)
...
xai_od_mean, xai_od_std = bootstrap_sample(value_list=np.array(xai_od_list_full), times=100)
od_fundus_mean, od_fundus_std = bootstrap_sample(value_list=np.array(od_fundus_list_full), times=100)
od_difference_mean, od_difference_std = bootstrap_sample(value_list=np.array(xai_od_list_full) -
                                                                    np.array(od_fundus_list_full), times=100)
iou_od_mean, iou_od_std = bootstrap_sample(value_list=np.array(iou_od_list_full), times=100)
dice_od_mean, dice_od_std = bootstrap_sample(value_list=np.array(dice_od_list_full), times=100)
hau_od_mean, hau_od_std = bootstrap_sample(value_list=np.array(hau_od_list_full), times=100)
...
xai_bv_mean, xai_bv_std = bootstrap_sample(value_list=np.array(xai_bv_list_full), times=100)
bv_fundus_mean, bv_fundus_std = bootstrap_sample(value_list=np.array(bv_fundus_list_full), times=100)
bv_difference_mean, bv_difference_std = bootstrap_sample(value_list=np.array(xai_bv_list_full) -
                                                                    np.array(bv_fundus_list_full), times=100)
iou_bv_mean, iou_bv_std = bootstrap_sample(value_list=np.array(iou_bv_list_full), times=100)
dice_bv_mean, dice_bv_std = bootstrap_sample(value_list=np.array(dice_bv_list_full), times=100)
hau_bv_mean, hau_bv_std = bootstrap_sample(value_list=np.array(hau_bv_list_full), times=100)
...
pd.DataFrame(results_df).to_csv("explanation_vgg_11_grad_cam_aggregation.csv", index=False, encoding="cp1252")
