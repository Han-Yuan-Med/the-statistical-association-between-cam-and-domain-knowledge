...
for id in range(len(dataset_id)):
    ...
    # Now using the SwinT
    SwinT_model = timm.create_model('swin_tiny_patch4_window7_224', pretrained=True)
    SwinT_model.head.fc = torch.nn.Linear(in_features=768, out_features=2, bias=True)
    print(SwinT_model.eval())

    # Move the input and SwinT_model to GPU if it is available
    SwinT_model.to(device)
    ...
    # Optimizer SGD
    optimizer = optim.SGD(filter(lambda p: p.requires_grad, SwinT_model.parameters()), lr=0.001, momentum=0.9)
    epoch_num = 100
    ...
    for epoch in range(epoch_num):  # loop over the dataset multiple times
        train_image(cls_model=SwinT_model, train_loader=train_loader, criterion=criterion,
        ...
        if auroc_value[epoch] > auroc_value_optimal:
            PATH = f"SwinT optimal {id}.pt"
            torch.save(SwinT_model, PATH)
...
for id in range(len(dataset_id)):
    ...
    results_df.append([f"Swin-Tiny", f"{id}", f"{thres_val}",
...
results_df.columns = ['Model', 'Data Split', 'Threshold', 'AUROC', 'AUPRC', 'Accuracy', 'Sensitivity', 'Specificity',
                      'PPV', 'NPV']
results_df.to_csv("results_swin_tiny_cls.csv", index=False, encoding="cp1252")
