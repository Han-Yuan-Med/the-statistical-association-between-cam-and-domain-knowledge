...
for id in range(len(dataset_id)):
    ...
    # Now using the DeiTT
    DeiTT_model = torch.hub.load('facebookresearch/deit:main', 'deit_tiny_patch16_224', pretrained=True)
    DeiTT_model.head = torch.nn.Sequential(
        torch.nn.Linear(in_features=192, out_features=2, bias=True), torch.nn.Softmax(dim=1))

    print(DeiTT_model.eval())

    # Move the input and DeiTT_model to GPU if it is available
    DeiTT_model.to(device)
    ...
    # Optimizer SGD
    optimizer = optim.SGD(filter(lambda p: p.requires_grad, DeiTT_model.parameters()), lr=0.001, momentum=0.9)
    epoch_num = 100
    ...
    for epoch in range(epoch_num):  # loop over the dataset multiple times
        train_image(cls_model=DeiTT_model, train_loader=train_loader, criterion=criterion,
        ...
        if auroc_value[epoch] > auroc_value_optimal:
            PATH = f"DeiTT optimal {id}.pt"
            torch.save(DeiTT_model, PATH)
