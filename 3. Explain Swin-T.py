...
for id in range(len(dataset_id)):
    ...
    SwinT_optimal = torch.load(f"SwinT optimal {id}.pt")
    target_layers = [SwinT_optimal.layers[-1].blocks[-1].norm2]
    targets = [ClassifierOutputTarget(1)]
    cam = GradCAM(model=SwinT_optimal, target_layers=target_layers,
    ...
    results_df.append([f"Swin-T", f"{id}", f"Grad-CAM", f"Optic cup",
    ...
    results_df.append([f"Swin-T", f"{id}", f"Grad-CAM", f"Optic disk",
    ...
    results_df.append([f"Swin-T", f"{id}", f"Grad-CAM", f"Blood vessels",
...
pd.DataFrame(results_df).to_csv("explanation_swin_t_grad_cam_cross_validation.csv", index=False, encoding="cp1252")
...
results_df_full.append([f"Swin-T", f"Grad-CAM", f"Optic cup",
                        f"{xai_oc_mean} ({xai_oc_std})", f"{oc_fundus_mean} ({oc_fundus_std})",
                        f"{oc_difference_mean} ({oc_difference_std})", f"{format(p_value, '.3e')}",
                        f"{iou_oc_mean} ({iou_oc_std})", f"{dice_oc_mean} ({dice_oc_std})",
                        f"{hau_oc_mean} ({hau_oc_std})"])
...
results_df_full.append([f"Swin-T", f"Grad-CAM", f"Optic disk",
                        f"{xai_od_mean} ({xai_od_std})",
                        f"{od_fundus_mean} ({od_fundus_std})",
                        f"{od_difference_mean} ({od_difference_std})", f"{format(p_value, '.3e')}",
                        f"{iou_od_mean} ({iou_od_std})", f"{dice_od_mean} ({dice_od_std})",
...
results_df_full.append([f"Swin-T", f"Grad-CAM", f"Blood vessels",
                        f"{xai_bv_mean} ({xai_bv_std})",
                        f"{bv_fundus_mean} ({bv_fundus_std})",
                        f"{bv_difference_mean} ({bv_difference_std})", f"{format(p_value, '.3e')}",
                        f"{iou_bv_mean} ({iou_bv_std})", f"{dice_bv_mean} ({dice_bv_std})",
...
pd.DataFrame(results_df_full).to_csv("explanation_swin_t_grad_cam_aggregation.csv", index=False, encoding="cp1252")
...
for id in range(len(dataset_id)):
    ...
    target_layers = [SwinT_optimal.layers[-1].blocks[-1].norm2]
    targets = [ClassifierOutputTarget(1)]
    cam = XGradCAM(model=SwinT_optimal, target_layers=target_layers,
                   reshape_transform=reshape_transform, use_cuda=True)
    ...
    results_df.append([f"Swin-T", f"{id}", f"XGrad-CAM", f"Optic cup",
                       f"{xai_oc_mean} ({xai_oc_std})", f"{oc_fundus_mean} ({oc_fundus_std})",
                       f"{oc_difference_mean} ({oc_difference_std})", f"{format(p_value, '.3e')}",
                       f"{iou_oc_mean} ({iou_oc_std})", f"{dice_oc_mean} ({dice_oc_std})",
                       f"{hau_oc_mean} ({hau_oc_std})"])
    ...
    results_df.append([f"Swin-T", f"{id}", f"XGrad-CAM", f"Optic disk",
                       f"{xai_od_mean} ({xai_od_std})",
                       f"{od_fundus_mean} ({od_fundus_std})",
                       f"{od_difference_mean} ({od_difference_std})", f"{format(p_value, '.3e')}",
                       f"{iou_od_mean} ({iou_od_std})", f"{dice_od_mean} ({dice_od_std})",
    ...
    results_df.append([f"Swin-T", f"{id}", f"XGrad-CAM", f"Blood vessels",
                       f"{xai_bv_mean} ({xai_bv_std})",
                       f"{bv_fundus_mean} ({bv_fundus_std})",
                       f"{bv_difference_mean} ({bv_difference_std})", f"{format(p_value, '.3e')}",
                       f"{iou_bv_mean} ({iou_bv_std})", f"{dice_bv_mean} ({dice_bv_std})",
...
pd.DataFrame(results_df).to_csv("explanation_swin_t_XGrad_cam_cross_validation.csv", index=False, encoding="cp1252")
...
results_df_full.append([f"Swin-T", f"XGrad-CAM", f"Optic cup",
                        f"{xai_oc_mean} ({xai_oc_std})", f"{oc_fundus_mean} ({oc_fundus_std})",
                        f"{oc_difference_mean} ({oc_difference_std})", f"{format(p_value, '.3e')}",
                        f"{iou_oc_mean} ({iou_oc_std})", f"{dice_oc_mean} ({dice_oc_std})",
                        f"{hau_oc_mean} ({hau_oc_std})"])
...
results_df_full.append([f"Swin-T", f"XGrad-CAM", f"Optic disk",
                        f"{xai_od_mean} ({xai_od_std})",
                        f"{od_fundus_mean} ({od_fundus_std})",
                        f"{od_difference_mean} ({od_difference_std})", f"{format(p_value, '.3e')}",
                        f"{iou_od_mean} ({iou_od_std})", f"{dice_od_mean} ({dice_od_std})",
...
results_df_full.append([f"Swin-T", f"XGrad-CAM", f"Blood vessels",
                        f"{xai_bv_mean} ({xai_bv_std})",
                        f"{bv_fundus_mean} ({bv_fundus_std})",
                        f"{bv_difference_mean} ({bv_difference_std})", f"{format(p_value, '.3e')}",
                        f"{iou_bv_mean} ({iou_bv_std})", f"{dice_bv_mean} ({dice_bv_std})",
...
pd.DataFrame(results_df_full).to_csv("explanation_swin_t_XGrad_cam_aggregation.csv", index=False, encoding="cp1252")
...
for id in range(len(dataset_id)):
    ...
    target_layers = [SwinT_optimal.layers[-1].blocks[-1].norm2]
    targets = [ClassifierOutputTarget(1)]
    cam = ScoreCAM(model=SwinT_optimal, target_layers=target_layers,
                   reshape_transform=reshape_transform, use_cuda=True)
    ...
    results_df.append([f"Swin-T", f"{id}", f"Score-CAM", f"Optic cup",
                       f"{xai_oc_mean} ({xai_oc_std})", f"{oc_fundus_mean} ({oc_fundus_std})",
                       f"{oc_difference_mean} ({oc_difference_std})", f"{format(p_value, '.3e')}",
                       f"{iou_oc_mean} ({iou_oc_std})", f"{dice_oc_mean} ({dice_oc_std})",
                       f"{hau_oc_mean} ({hau_oc_std})"])
    ...
    results_df.append([f"Swin-T", f"{id}", f"Score-CAM", f"Optic disk",
                       f"{xai_od_mean} ({xai_od_std})",
                       f"{od_fundus_mean} ({od_fundus_std})",
                       f"{od_difference_mean} ({od_difference_std})", f"{format(p_value, '.3e')}",
                       f"{iou_od_mean} ({iou_od_std})", f"{dice_od_mean} ({dice_od_std})",
    ...
    results_df.append([f"Swin-T", f"{id}", f"Score-CAM", f"Blood vessels",
                       f"{xai_bv_mean} ({xai_bv_std})",
                       f"{bv_fundus_mean} ({bv_fundus_std})",
                       f"{bv_difference_mean} ({bv_difference_std})", f"{format(p_value, '.3e')}",
                       f"{iou_bv_mean} ({iou_bv_std})", f"{dice_bv_mean} ({dice_bv_std})",
...
pd.DataFrame(results_df).to_csv("explanation_swin_t_Score_cam_cross_validation.csv", index=False, encoding="cp1252")
...
results_df_full.append([f"Swin-T", f"Score-CAM", f"Optic cup",
                        f"{xai_oc_mean} ({xai_oc_std})", f"{oc_fundus_mean} ({oc_fundus_std})",
                        f"{oc_difference_mean} ({oc_difference_std})", f"{format(p_value, '.3e')}",
                        f"{iou_oc_mean} ({iou_oc_std})", f"{dice_oc_mean} ({dice_oc_std})",
                        f"{hau_oc_mean} ({hau_oc_std})"])
...
results_df_full.append([f"Swin-T", f"Score-CAM", f"Optic disk",
                        f"{xai_od_mean} ({xai_od_std})",
                        f"{od_fundus_mean} ({od_fundus_std})",
                        f"{od_difference_mean} ({od_difference_std})", f"{format(p_value, '.3e')}",
                        f"{iou_od_mean} ({iou_od_std})", f"{dice_od_mean} ({dice_od_std})",
...
results_df_full.append([f"Swin-T", f"Score-CAM", f"Blood vessels",
                        f"{xai_bv_mean} ({xai_bv_std})",
                        f"{bv_fundus_mean} ({bv_fundus_std})",
                        f"{bv_difference_mean} ({bv_difference_std})", f"{format(p_value, '.3e')}",
                        f"{iou_bv_mean} ({iou_bv_std})", f"{dice_bv_mean} ({dice_bv_std})",
...
pd.DataFrame(results_df_full).to_csv("explanation_swin_t_Score_cam_aggregation.csv", index=False, encoding="cp1252")
...
for id in range(len(dataset_id)):
    ...
    target_layers = [SwinT_optimal.layers[-1].blocks[-1].norm2]
    targets = [ClassifierOutputTarget(1)]
    cam = EigenCAM(model=SwinT_optimal, target_layers=target_layers,
                   reshape_transform=reshape_transform, use_cuda=True)
    ...
    results_df.append([f"Swin-T", f"{id}", f"Eigen-CAM", f"Optic cup",
                       f"{xai_oc_mean} ({xai_oc_std})", f"{oc_fundus_mean} ({oc_fundus_std})",
                       f"{oc_difference_mean} ({oc_difference_std})", f"{format(p_value, '.3e')}",
                       f"{iou_oc_mean} ({iou_oc_std})", f"{dice_oc_mean} ({dice_oc_std})",
                       f"{hau_oc_mean} ({hau_oc_std})"])
    ...
    results_df.append([f"Swin-T", f"{id}", f"Eigen-CAM", f"Optic disk",
                       f"{xai_od_mean} ({xai_od_std})",
                       f"{od_fundus_mean} ({od_fundus_std})",
                       f"{od_difference_mean} ({od_difference_std})", f"{format(p_value, '.3e')}",
                       f"{iou_od_mean} ({iou_od_std})", f"{dice_od_mean} ({dice_od_std})",
    ...
    results_df.append([f"Swin-T", f"{id}", f"Eigen-CAM", f"Blood vessels",
                       f"{xai_bv_mean} ({xai_bv_std})",
                       f"{bv_fundus_mean} ({bv_fundus_std})",
                       f"{bv_difference_mean} ({bv_difference_std})", f"{format(p_value, '.3e')}",
                       f"{iou_bv_mean} ({iou_bv_std})", f"{dice_bv_mean} ({dice_bv_std})",
...
pd.DataFrame(results_df).to_csv("explanation_swin_t_Eigen_cam_cross_validation.csv", index=False, encoding="cp1252")
...
results_df_full.append([f"Swin-T", f"Eigen-CAM", f"Optic cup",
                        f"{xai_oc_mean} ({xai_oc_std})", f"{oc_fundus_mean} ({oc_fundus_std})",
                        f"{oc_difference_mean} ({oc_difference_std})", f"{format(p_value, '.3e')}",
                        f"{iou_oc_mean} ({iou_oc_std})", f"{dice_oc_mean} ({dice_oc_std})",
                        f"{hau_oc_mean} ({hau_oc_std})"])
...
results_df_full.append([f"Swin-T", f"Eigen-CAM", f"Optic disk",
                        f"{xai_od_mean} ({xai_od_std})",
                        f"{od_fundus_mean} ({od_fundus_std})",
                        f"{od_difference_mean} ({od_difference_std})", f"{format(p_value, '.3e')}",
                        f"{iou_od_mean} ({iou_od_std})", f"{dice_od_mean} ({dice_od_std})",
...
results_df_full.append([f"Swin-T", f"Eigen-CAM", f"Blood vessels",
                        f"{xai_bv_mean} ({xai_bv_std})",
                        f"{bv_fundus_mean} ({bv_fundus_std})",
                        f"{bv_difference_mean} ({bv_difference_std})", f"{format(p_value, '.3e')}",
                        f"{iou_bv_mean} ({iou_bv_std})", f"{dice_bv_mean} ({dice_bv_std})",
...
pd.DataFrame(results_df_full).to_csv("explanation_swin_t_Eigen_cam_aggregation.csv", index=False, encoding="cp1252")
...
for id in range(len(dataset_id)):
    ...
    target_layers = [SwinT_optimal.layers[-1].blocks[-1].norm2]
    targets = [ClassifierOutputTarget(1)]
    cam = LayerCAM(model=SwinT_optimal, target_layers=target_layers,
                   reshape_transform=reshape_transform, use_cuda=True)
    ...
    results_df.append([f"Swin-T", f"{id}", f"Layer-CAM", f"Optic cup",
                       f"{xai_oc_mean} ({xai_oc_std})", f"{oc_fundus_mean} ({oc_fundus_std})",
                       f"{oc_difference_mean} ({oc_difference_std})", f"{format(p_value, '.3e')}",
                       f"{iou_oc_mean} ({iou_oc_std})", f"{dice_oc_mean} ({dice_oc_std})",
                       f"{hau_oc_mean} ({hau_oc_std})"])
    ...
    results_df.append([f"Swin-T", f"{id}", f"Layer-CAM", f"Optic disk",
                       f"{xai_od_mean} ({xai_od_std})",
                       f"{od_fundus_mean} ({od_fundus_std})",
                       f"{od_difference_mean} ({od_difference_std})", f"{format(p_value, '.3e')}",
                       f"{iou_od_mean} ({iou_od_std})", f"{dice_od_mean} ({dice_od_std})",
    ...
    results_df.append([f"Swin-T", f"{id}", f"Layer-CAM", f"Blood vessels",
                       f"{xai_bv_mean} ({xai_bv_std})",
                       f"{bv_fundus_mean} ({bv_fundus_std})",
                       f"{bv_difference_mean} ({bv_difference_std})", f"{format(p_value, '.3e')}",
                       f"{iou_bv_mean} ({iou_bv_std})", f"{dice_bv_mean} ({dice_bv_std})",
...
pd.DataFrame(results_df).to_csv("explanation_swin_t_Layer_cam_cross_validation.csv", index=False, encoding="cp1252")
...
results_df_full.append([f"Swin-T", f"Layer-CAM", f"Optic cup",
                        f"{xai_oc_mean} ({xai_oc_std})", f"{oc_fundus_mean} ({oc_fundus_std})",
                        f"{oc_difference_mean} ({oc_difference_std})", f"{format(p_value, '.3e')}",
                        f"{iou_oc_mean} ({iou_oc_std})", f"{dice_oc_mean} ({dice_oc_std})",
                        f"{hau_oc_mean} ({hau_oc_std})"])
...
results_df_full.append([f"Swin-T", f"Layer-CAM", f"Optic disk",
                        f"{xai_od_mean} ({xai_od_std})",
                        f"{od_fundus_mean} ({od_fundus_std})",
                        f"{od_difference_mean} ({od_difference_std})", f"{format(p_value, '.3e')}",
                        f"{iou_od_mean} ({iou_od_std})", f"{dice_od_mean} ({dice_od_std})",
...
results_df_full.append([f"Swin-T", f"Layer-CAM", f"Blood vessels",
                        f"{xai_bv_mean} ({xai_bv_std})",
                        f"{bv_fundus_mean} ({bv_fundus_std})",
                        f"{bv_difference_mean} ({bv_difference_std})", f"{format(p_value, '.3e')}",
                        f"{iou_bv_mean} ({iou_bv_std})", f"{dice_bv_mean} ({dice_bv_std})",
...
pd.DataFrame(results_df_full).to_csv("explanation_swin_t_Layer_cam_aggregation.csv", index=False, encoding="cp1252")
